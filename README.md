...
- Supports event counting at rates up to 25.5 MHz
...
In the current design, the ADC clock is operating at 125 MHz (max achievable in the Red Pitaya 125-14 board), and the fpga logic is deliberately run at a different clock to demonstrate the robust implementation of the system. To demonstrate the max clock speed at which the FPGA logic still meets the  timing constraints, the FPGA is successfully operated at 250 MHz. The FPGA supports a counting of input events upto 25.5 MHz, basically limited by the 8-bit implementation of the counter RAM memory, and the counting bin window of 10 &micro;s.
